...
The original Level 8 evolution framework now prototypes **Striker**, **Spinner**, and **Rammer** as enabled, mutually exclusive per-life choices. The progression foundation defines evolution gates at Levels **8, 20, 35, 75, 150, 250, and 300**. Level 20 retains provisional branch data, while Levels 35 and later are route-data placeholders with no designed or implemented robots. The current test must establish that Striker's burst-and-shove hammer, Spinner's sustained contact pressure, and Rammer's committed hold-and-release frontal charge create readable original counterplay without weakening server authority.
...
The current bounded milestone is the Level 300 progression foundation documented in `docs/LEVEL300_PROGRESSION_FOUNDATION.md`: score-only levels beyond 300, seven evolution gates, an 80-point schedule, a faster provisional XP curve, and exact one-time elimination transfer of unbanked run XP. Later robots are deliberately absent. After multiplayer, security, HUD, death/disconnect, extraction, and human pacing gates pass, the next class milestone may implement Level 20 route selection and its branching graph. Functional skins, live leaderboards, matchmaking, extra modes, death choices, paid revives, ads, spending systems, monetization, and production persistence launch remain separate future milestones.
