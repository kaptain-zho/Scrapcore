...
The internal Impact Foundry prototype proves generic Level 20 route commitment and complete kit replacement without changing Level 8 tuning. LMB begins one server-timed forge strike with reduced windup movement, an automatic bounded frontal impact, and normal punishable recovery. Only frontal horizontal displacement is partially resisted during the windup; damage, side and rear displacement, vertical motion, and wall collision remain normal.

The internal Motion Works prototype adds an energy-limited powered carriage without changing Level 8 or Impact Foundry tuning. Holding LMB completes a server-timed spool, enters a bounded forward Driven phase, drains energy, ramps output, and reduces steering as output rises. Releasing, losing the keepalive lease, reaching zero energy, hitting a solid obstruction, dying, extracting, or leaving the arena ends damage and enters coastdown/recovery. Narrow server queries deal repeated low-shove damage only while the authoritative chassis is moving forward in the legal weapon lane. Ordinary players still see Level 20 routes as under construction. Field Rig must be implemented and all three families must pass multiplayer, security, farming, input, and balance gates before public Level 20 selection can be considered.
